...
export function StatCardsRow({ className = '', stats, isAuthenticated }: StatCardsRowProps) {
  const cards = [
    {
      id: 'setups',
      label: 'Setups',
      value: isAuthenticated ? String(stats?.setupsThisMonth ?? 0) : '—',
      sub: isAuthenticated ? 'This month' : 'Sign in to track',
      icon: Zap,
      ...
      iconBezelClass:
        'bg-okx-violet/12 ring-1 ring-inset ring-okx-violet/25 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]',
    },
    {
      id: 'winrate',
      label: 'Win rate',
      value: isAuthenticated ? `${(stats?.winRate ?? 0).toFixed(1)}%` : '—',
      sub: isAuthenticated ? `Last ${stats?.closedTrades ?? 0} closed` : 'Sign in to track',
      icon: Target,
      ...
      iconBezelClass:
        'bg-okx-cyan/12 ring-1 ring-inset ring-okx-cyan/25 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]',
    },
    {
      id: 'pnl',
      label: 'Total P&L',
      value: isAuthenticated ? formatPnl(stats?.totalPnl ?? 0) : '—',
      sub: 'USD · realized',
      icon: TrendingUp,
      ...
      iconBezelClass:
        'bg-okx-amber/12 ring-1 ring-inset ring-okx-amber/25 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]',
    },
    {
      id: 'rr',
      label: 'Avg win / loss',
      value: isAuthenticated ? `${(stats?.riskReward ?? 0).toFixed(1)} R` : '—',
      sub: 'Risk-reward',
      icon: BarChart3,
      ...
      iconBezelClass:
        'bg-white/6 ring-1 ring-inset ring-white/12 shadow-[inset_0_1px_0_rgba(255,255,255,0.1)]',
    },
    {
      id: 'last',
      label: 'Last trade',
      value: isAuthenticated ? formatPnl(stats?.lastTradePnl ?? null) : '—',
      sub: 'Closed · profit',
      icon: Activity,
      ...
      iconBezelClass:
        'bg-okx-teal/12 ring-1 ring-inset ring-okx-teal/25 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]',
    },
    {
      id: 'streak',
      label: 'Win streak',
      value: isAuthenticated ? `${stats?.winStreak ?? 0}W` : '—',
      sub: 'Current streak',
      icon: ArrowUpRight,
      ...
      iconBezelClass:
        'bg-okx-rose/12 ring-1 ring-inset ring-okx-rose/25 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]',
    },
  ]
          ...
          >
            ...
            <div className="stat-glass-card__content">
              <div className="flex items-center justify-between gap-2">
                <span className="text-[11px] font-medium tracking-wide text-okx-muted">
                  {s.label}
                </span>
                <div
